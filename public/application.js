...
var config = {
  apiOrigins: {
    production: 'https://fast-taiga-19098.herokuapp.com'
  }
};
...
var showIndexItemsTemplate = __webpack_require__(22);
var showIndexListTemplate = __webpack_require__(39);
var showNewItemTemplate = __webpack_require__(40);
var showNewListTemplate = __webpack_require__(41);
var showSearchItemTemplate = __webpack_require__(42);
var showEditListTemplate = __webpack_require__(43);
var showListPartialTemplate = __webpack_require__(44);
var showSearchResultsTemplate = __webpack_require__(45);
var showEditItemTemplate = __webpack_require__(46);

var store = __webpack_require__(3);

var indexItems = function indexItems(data) {
  for (var key in data) {
    data[key]['owner'] = data[key]['user_id'] === store.user.id;
  }
  var showNewItemHtml = showIndexItemsTemplate({ items: data });
  showContent(showNewItemHtml);
};

var indexLists = function indexLists(data) {
  var showIndexListsHtml = showIndexListTemplate({ lists: data });
  showContent(showIndexListsHtml);
};

var newItem = function newItem() {
  var showNewItemHtml = showNewItemTemplate();
  showContent(showNewItemHtml);
};

var newList = function newList() {
  var showNewlistHtml = showNewListTemplate();
  showContent(showNewlistHtml);
};

var searchItems = function searchItems(data) {
  var showSearchItemsHtml = showSearchItemTemplate({ 'list': store.list });
  showContent(showSearchItemsHtml);
};

var editItem = function editItem(data) {
  var showEditItemHtml = showEditItemTemplate({ 'item': data });
  showContent(showEditItemHtml);
};

var searchItemResults = function searchItemResults(data) {
  var showSearchResultsHtml = showSearchResultsTemplate({ items: data });
  $('div[data-results="items"]').empty();
  $('div[data-results="items"]').append(showSearchResultsHtml);
};

var editList = function editList(data) {
  store.list = null;
  var showEditListHtml = showEditListTemplate({ list: data });
  showContent(showEditListHtml);
};

var partialList = function partialList(data) {
  var showListPartialHtml = showListPartialTemplate({ lists: data });
  $('td[data-content="target"]').empty();
  $('td[data-content="target"]').append(showListPartialHtml);
};

var showContent = function showContent(html) {
  $('#content').empty();
  $('#content').append(html);
};
...
var indexLists = function indexLists() {
  return $.ajax({
...
var updateList = function updateList(id, data) {
  return $.ajax({
    url: config.apiOrigin + '/lists/' + id,
    method: 'patch',
    data: data,
...
var onIndexSuccess = function onIndexSuccess(response, status, xhr) {
  handlebars.indexLists(response.lists);
};

var onListNames = function onListNames(response, status, xhr) {
  handlebars.partialList(response.lists);
};

var onIndexFailure = function onIndexFailure(response, status, xhr) {};

var onGetSuccess = function onGetSuccess(response, status, xhr) {
  listDateFix(response.list);
};

var onToList = function onToList(response, status, xhr) {
  handlebars.partialList({ 'lists': response.list });
};

var onGetFailure = function onGetFailure(response, status, xhr) {};

var onCreateSuccess = function onCreateSuccess(response, status, xhr) {
  listDateFix(response.list);
};

var onCreateFailure = function onCreateFailure(response, status, xhr) {};

var onUpdateSuccess = function onUpdateSuccess(response, status, xhr) {
  listDateFix(response.list);
};

var onUpdateFailure = function onUpdateFailure(response, status, xhr) {};

var listDateFix = function listDateFix(data) {
...
var onAddItemToList = function onAddItemToList(data) {
  return $.ajax({
...
var listsApi = __webpack_require__(9);
var listsUi = __webpack_require__(10);

var onDeleteSuccess = function onDeleteSuccess(response, status, xhr) {};

var onDeleteFailure = function onDeleteFailure(response, status, xhr) {};

var onCreateSuccess = function onCreateSuccess(response, status, xhr) {
  listsApi.getList(response.list_item.list.id).then(listsUi.onGetSuccess).catch(listsUi.onGetFailure);
};
...
var authApi = __webpack_require__(20);
var authUI = __webpack_require__(21);
var handlebars = __webpack_require__(6);
var store = __webpack_require__(3);
var listApi = __webpack_require__(9);
var listUI = __webpack_require__(10);
var itemApi = __webpack_require__(47);
var itemUI = __webpack_require__(48);
var liApi = __webpack_require__(11);
var liUI = __webpack_require__(12);

var signOutUser = function signOutUser(event) {
  event.preventDefault();
  authApi.signOut().then(authUI.signOutSuccess).catch(authUI.signOutFailure);
};

var signInUser = function signInUser(event) {
...
var tryCollapse = function tryCollapse() {
  if ($(window).width() <= 767) {
    $('#siteNavbar').collapse('toggle');
  }
};

var navHandler = function navHandler(event) {
  event.preventDefault();
  var handleToGet = this.attributes['data-view'].value;
  switch (handleToGet) {
    case 'new-item':
      {
        handlebars.newItem();
...
var indexLists = function indexLists() {
  listApi.indexLists().then(listUI.onIndexSuccess).catch(listUI.onIndexFailure);
};

var toggleLIPurchased = function toggleLIPurchased(event) {
  var listitem = $(event.target).attr('data-listitem');
  var data = { 'list_item': { 'purchased': event.target.checked } };
  liApi.onUpdate(listitem, data).then(liUI.onUpdateSuccess).catch(liUI.onUpdateFailure);
};

var toggleListActive = function toggleListActive(event) {
  var id = $(event.target).attr('data-list');
  var active = event.target.checked || false;
  updateListActive(id, active);
};

var updateListActive = function updateListActive(id, active) {
  listApi.updateList(id, { 'list': { 'active': active } }).then(listUI.onUpdateSuccess).catch(listUI.onUpdateFailure);
};

var toggleShownLists = function toggleShownLists(event) {
  event.preventDefault();
...
var createNewList = function createNewList(event) {
  event.preventDefault();
  var data = getFormFields(this);
  listApi.createList(data).then(listUI.onCreateSuccess).catch(listUI.onCreateFailure);
};

var createNewItem = function createNewItem(event) {
  event.preventDefault();
  var data = getFormFields(this);
  itemApi.createItem(data).then(itemUI.onCreateSuccess).catch(itemUI.onCreateFailure);
};

var editList = function editList(event) {
  var id = this.attributes['data-listid'].value;
  listApi.getList(id).then(listUI.onGetSuccess).catch(listUI.onGetFailure);
};

var toggleListGroupEdit = function toggleListGroupEdit(event) {
  var dataNote = event.target.attributes['data-note'].value;
  switch (dataNote) {
    case 'edit-group':
      {
    ...
    case 'save-group':
      {
        var baseName = $('input[data-base]').attr('data-base');
        var saveName = $('input[data-base]').val();
        ...
        if (baseName !== saveName) {
          var data = { 'list': { 'name': saveName } };
          var id = $('#content div[data-id]').attr('data-id');
          listApi.updateList(id, data).then(listUI.onUpdateSuccess).catch(listUI.onUpdateFailure);
        }
        break;
      }
  }
...
var editListRow = function editListRow(event) {
  var cause = $(event.target).text();
  var listitem = $(event.target).attr('data-listitem');
  var itemId = $(event.target).attr('data-item');
  var listId = $(event.target).closest('table').attr('data-list');
  switch (cause) {
    case 'Edit':
      {
    ...
    case 'Save':
      {
        var saveValue = $('input[data-note="edit-li"][data-listitem="' + listitem + '"]').val();
        var data = { 'list_item': { 'list_id': listId, 'item_id': itemId, 'purchased': false, 'quantity': saveValue } };
        liApi.onUpdate(listitem, data).then(liUI.onUpdateSuccess).catch(liUI.onUpdateFailure);
        break;
      }
    default:
      {
        ...
        liApi.deleteListItem(listitem).then(liUI.onDeleteSuccess).catch(liUI.onDeleteFailure);
        break;
      }
  }
};

var addItemToList = function addItemToList(event) {
  event.preventDefault();
  var itemID = $(event.target).attr('data-item');
  ...
  if (store.list) {
    listApi.getList(store.list).then(listUI.onToList).catch(listUI.onIndexFailure);
  } else {
    listApi.indexLists().then(listUI.onListNames).catch(listUI.onIndexFailure);
  }
};

var showQuantity = function showQuantity(event) {
  var row = $(event.target).attr('data-showedit');
  $('tr[data-row="' + row + '"] td').removeClass('hidden');
  $(event.target).removeClass('btn-success').addClass('btn-secondary');
};

var appendItemToList = function appendItemToList(event) {
  var listId = $(event.target).attr('data-to');
  var itemId = $(event.target).closest('table').parent().attr('data-item');
  var quantity = $(event.target).parent().prev().children('input').val();
  var data = { 'list_item': { 'list_id': listId, 'item_id': itemId, 'purchased': false, 'quantity': quantity } };
  liApi.onAddItemToList(data).then(liUI.onCreateSuccess).catch(liUI.onCreateFailure);
};

var searchItems = function searchItems(event) {
  var searchBase = $(event.target).attr('data-search');
  var searchCriteria = $('input[data-search="' + searchBase + '"]').val();
  switch (searchBase) {
    case 'items':
      {
        itemApi.onSearchItems(searchCriteria).then(itemUI.onSearchSuccess).catch(itemUI.onSearchFailure);
        break;
      }
  }
};
...
var onIndexItems = function onIndexItems(event) {
  event.preventDefault();
  itemApi.indexItems().then(itemUI.onIndexSuccess).catch(itemUI.onIndexFailure);
};

var searchToList = function searchToList(event) {
  store.list = $(event.target).closest('div').attr('data-id');
  handlebars.searchItems();
};

var newToList = function newToList(event) {
  store.list = $(event.target).closest('div').attr('data-id');
  handlebars.newItem();
};
...
var deleteList = function deleteList(event) {
  var id = $(event.target).attr('data-list');
  listApi.deleteList(id).then(indexLists);
};

var showEditItem = function showEditItem(event) {
  var id = $(event.target).attr('data-edit-item');
  itemApi.getItem(id).then(itemUI.onGetSuccess).catch(itemUI.onGetFailure);
};
...
var editItem = function editItem(event) {
  ...
  itemApi.onUpdate(id, data).then(itemUI.onUpdateSuccess).catch(itemUI.onUpdateFailure);
};
...
var addHandlers = function addHandlers() {
  ...
  $('#content').on('click', 'button[data-new-list]', function () {
    handlebars.newList();
  });
...
// Sign Up promises
var signUpSuccess = function signUpSuccess(response, status, xhr) {
...
var signUpFailure = function signUpFailure(response, status, xhr) {
  $('#signUpComment').html('<br/><p><mark>Sorry, those login credentials cannot be used.</mark></p>');
};

// Sign In promises
var signInSuccess = function signInSuccess(response, status, xhr) {
  $('#login-modal').modal('hide');
  store.user = response.user;
  toggleUserDisplay(true);
  clearModals();
};
var signInFailure = function signInFailure(response, status, xhr) {
  $('#signInComment').html('<br/><p><mark>Please check login credentials and try again.</mark></p>');
};

// Change Password promises
var changePasswordSuccess = function changePasswordSuccess(response, status, xhr) {
  $('#changeComment').html('<br/><p>Password change successful.</p>');
  clearModals();
};

var changePasswordFailure = function changePasswordFailure(response, status, xhr) {
  $('#changeComment').html('<br/><p><mark>Could not update password.</mark></p>');
};

var signOutSuccess = function signOutSuccess(response, status, xhr) {
  store.user = null;
  $('#content').empty();
  toggleUserDisplay(false);
};

var signOutFailure = function signOutFailure(response, status, xhr) {};
...
var toggleUserDisplay = function toggleUserDisplay(check) {
  if (check) {
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "    <tr data-item=\""
    ...
    + "\n      </td>\n      <td>\n        "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.unit : stack1), depth0))
    + "\n      </td>\n      <td>\n        "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.unit_quantity : stack1), depth0))
    + "\n      </td>\n      <td>\n        <button title=\"Add to a list\" data-item=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"btn-sm btn-warning\">&#43;</button>\n        "
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.owner : stack1),{"name":"if","hash":{},"fn":container.program(2, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "\n      </td>\n    </tr>\n    <tr class=\"hidden\" data-itemhide data-item=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\"><td colspan=\"4\" data-item=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\"><td>\n    </tr>\n";
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "<button title=\"Edit item\" data-edit-item=\""
    + container.escapeExpression(container.lambda(((stack1 = blockParams[1][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"btn-sm btn-primary\">Edit</button>";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "<table class=\"table table-hover table-responsive\">\n  <caption>You may use any item, but only edit your own.</caption>\n  <thead>\n    <tr>\n      <th>\n        Item Name\n      </th>\n      <th>\n        Unit of Measure\n      </th>\n      <th>\n        Quantity per unit\n      </th>\n      <th>\n      </th>\n    </tr>\n  </thead>\n"
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<h3>Now showing only active lists.</h3>\n<a href=\"#\" data-show=\"active\">Show all</a>\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.lists : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=depth0 != null ? depth0 : (container.nullContext || {}), alias2=container.lambda, alias3=container.escapeExpression;

  return "  <div class=\"container-fluid "
    + ((stack1 = helpers["if"].call(alias1,((stack1 = blockParams[0][0]) != null ? stack1.active : stack1),{"name":"if","hash":{},"fn":container.program(3, data, 0, blockParams),"inverse":container.program(5, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "\" data-list=\""
    + ((stack1 = helpers["if"].call(alias1,((stack1 = blockParams[0][0]) != null ? stack1.active : stack1),{"name":"if","hash":{},"fn":container.program(7, data, 0, blockParams),"inverse":container.program(9, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "\">\n  <table class=\"table table-responsive table-hover\">\n    <caption>List "
    + alias3(alias2(((stack1 = blockParams[0][0]) != null ? stack1.name : stack1), depth0))
    + " Active   <input type=\"checkbox\" "
    + ((stack1 = helpers["if"].call(alias1,((stack1 = blockParams[0][0]) != null ? stack1.active : stack1),{"name":"if","hash":{},"fn":container.program(11, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + " data-list=\""
    + alias3(alias2(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n      <thead>\n        <tr>\n          <th>\n            Item Name\n          </th>\n          <th>\n            Needed\n          </th>\n          <th>\n            Package\n          </th>\n          <th>\n            Purchased?\n          </th>\n        </tr>\n      </thead>\n"
    + ((stack1 = helpers.each.call(alias1,((stack1 = blockParams[0][0]) != null ? stack1.list_items : stack1),{"name":"each","hash":{},"fn":container.program(13, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "  </table>\n  <button class=\"btn-sm btn-default\" data-listid="
    + alias3(alias2(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + ">Edit\n  </button>\n</div>\n";
  ...
  return "      <tr>\n        <td>\n          "
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.name : stack1), depth0))
    + "\n        </td>\n        <td>\n          "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.quantity : stack1), depth0))
    + "\n        </td>\n        <td>\n          "
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.unit_quantity : stack1), depth0))
    + " "
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.unit : stack1), depth0))
    + "\n        </td>\n        <td>\n          <input type=\"checkbox\" "
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.purchased : stack1),{"name":"if","hash":{},"fn":container.program(11, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + " data-listitem=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n        </td>\n      </tr>\n";
},"15":function(container,depth0,helpers,partials,data) {
    return "<h3>You have no lists.</h3>\nCreate a new list? <button class=\"btn btn-xs btn-success\" data-new-list>New List</button>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.lists : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.program(15, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    return "\n<form id=\"new-item-form\">\n  <h1>Create a New Item</h1>\n  <table class=\"table table-hover table-responsive\">\n    <thead>\n      <tr>\n        <th>\n          Item Name\n        </th>\n        <th>\n          Unit of Measure\n        </th>\n        <th>\n          Quantity per Unit\n        </th>\n      </tr>\n    </thead>\n    <tr>\n      <td>\n        <input type=\"text\" name=\"item[name]\" placeholder=\"Item Name\" >\n      </td>\n      <td>\n        <input type=\"text\" name=\"item[unit]\" placeholder=\"Unit of Measure\">\n      </td>\n      <td>\n        <input type=\"number\" name=\"item[unit_quantity]\" placeholder=\"Item Name\" min=\"1\"A>\n      </td>\n    </tr>\n  </table>\n  <input class=\"btn-primary\" type=\"submit\" value=\"Submit &raquo;\">\n  <button data-item-index class=\"btn-default\">Cancel\n  </button>\n</form>\n<div id=\"new-item-response\"></div>\n";
},"useData":true});
...
function __default(obj) { return obj && (obj.__esModule ? obj["default"] : obj); }
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    return "\n<form id=\"new-list-form\">\n  <h1>Create a New List</h1>\n  <p>List Name: <input type=\"text\" name=\"list[name]\" placeholder=\"List Name\" ></p>\n  <br/>\n  <p>This list will be automatically marked \"Active\".<input class=\"hidden\" type=\"text\" name=\"list[active]\" value=\"true\" ></p>\n  <input class=\"btn-primary\" type=\"submit\" value=\"Submit &raquo;\">\n  <button class=\"btn-default\">Cancel</button>\n</form>\n";
},"useData":true});
...
function __default(obj) { return obj && (obj.__esModule ? obj["default"] : obj); }
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    return "<h1>Find an item</h1>\n<p>Item name? <input type=\"text\" data-search=\"items\" placeholder=\"Enter a search word.\"></p>\n<button class=\"btn-primary\" data-search=\"items\"><i class=\"glyphicon glyphicon-search\"></i> Search &raquo;</button>\n<div data-results=\"items\"></div>\n";
},"useData":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "      <tr data-listitem=\""
    ...
    + "</p>\n          <input data-listitem=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" data-note=\"edit-li\" class=\"hidden\" type=\"number\" name=\"list_item[quantity]\" value=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.quantity : stack1), depth0))
    + "\">\n        </td>\n        <td>\n          "
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.unit_quantity : stack1), depth0))
    + " "
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.unit : stack1), depth0))
    + "\n        </td>\n        <td>\n          <input type=\"checkbox\" "
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.purchased : stack1),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + " data-listitem=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n        </td>\n        <td>\n          <button data-note=\"edit-li\" data-listitem=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" data-item=\""
    + alias2(alias1(((stack1 = ((stack1 = blockParams[0][0]) != null ? stack1.item : stack1)) != null ? stack1.id : stack1), depth0))
    + "\" class=\"btn-sm btn-warning\">Edit</button>\n        </td>\n        <td>\n          <button data-note=\"edit-li\" data-listitem=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"btn-sm btn-danger\">&times;\n          </button>\n        </td>\n      </tr>\n";
},"5":function(container,depth0,helpers,partials,data) {
    var stack1;

  return ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.complete : stack1),{"name":"if","hash":{},"fn":container.program(6, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "");
},"6":function(container,depth0,helpers,partials,data) {
    var stack1;

  return ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.active : stack1),{"name":"if","hash":{},"fn":container.program(7, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "");
},"7":function(container,depth0,helpers,partials,data) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

  return "        <p>This list is complete, would you like to delete it or save it as inactive?</p>\n        <button data-delete-list class=\"btn btn-danger\" data-list=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.id : stack1), depth0))
    + "\">Delete</button><button data-save-list class=\"btn btn-info\" data-list=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.id : stack1), depth0))
    + "\">Save</button>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression, alias3=depth0 != null ? depth0 : (container.nullContext || {});

  return "<h3>Now editing your "
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.name : stack1), depth0))
    + " list. Last updated on "
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.updated_at : stack1), depth0))
    + "</h3>\n<div class=\"container-fluid\" data-id=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.id : stack1), depth0))
    + "\">\n  <div>\n    <p data-note=\"edit-group\">\n      Current list name: <span data-id=\"group-name\">"
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.name : stack1), depth0))
    + "</span>.\n    </p>\n    <input data-base=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.name : stack1), depth0))
    + "\" data-note=\"edit-group\" class=\"hidden\" type=\"text\" name=\"list[name]\" value=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.name : stack1), depth0))
    + "\">\n    <a href=\"#\" data-note=\"edit-group\">\n      Change\n    </a>\n  </div>\n  <table class=\"table table-responsive table-hover\" data-list=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.id : stack1), depth0))
    + "\">\n    <caption>\n      List "
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.name : stack1), depth0))
    + " Active   <input type=\"checkbox\" "
    + ((stack1 = helpers["if"].call(alias3,((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.active : stack1),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + " data-list=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.id : stack1), depth0))
    + "\">\n    </caption>\n    <thead>\n      <tr>\n        <th>\n          Item Name\n        </th>\n        <th>\n          Needed\n        </th>\n        <th>\n          Package\n        </th>\n        <th>\n          Purchased?\n        </th>\n        <th></th>\n        <th></th>\n      </tr>\n    </thead>\n"
    + ((stack1 = helpers.each.call(alias3,((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.list_items : stack1),{"name":"each","hash":{},"fn":container.program(3, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    ...
    + ((stack1 = helpers["if"].call(alias3,((stack1 = (depth0 != null ? depth0.list : depth0)) != null ? stack1.list_items : stack1),{"name":"if","hash":{},"fn":container.program(5, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "</div>\n";
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.lists : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;
  ...
  return "    <tr data-row=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" "
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.active : stack1),{"name":"if","hash":{},"fn":container.program(3, data, 0, blockParams),"inverse":container.program(5, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + " >\n      <td>\n        "
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.name : stack1), depth0))
    + "\n      </td>\n      <td class=\"hidden\" data-list=\"item\">\n        How many? <input type=\"number\" placeholder=\"Needed\">\n      </td>\n      <td class=\"hidden\" data-list=\"item\">\n        <button data-to=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"btn-sm btn btn-success\">&#43;\n        </button>\n      </td>\n      <td>\n        <button data-showedit=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"btn-sm btn btn-success\" title=\"This list\" data-function=\"quantity\">&#10004;\n        </button>\n      </td>\n    </tr>\n";
},"3":function(container,depth0,helpers,partials,data) {
    return "";
},"5":function(container,depth0,helpers,partials,data) {
    return "class=\"hidden\"";
},"7":function(container,depth0,helpers,partials,data) {
    return "    <tr>\n      <td>\n        You have no lists. <button class=\"btn btn-xs btn-success\" data-new-list >Make a new list &raquo;</button>\n      </td>\n    </tr>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "<table class=\"table table-hover table-responsive\">\n"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.lists : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.program(7, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "</table>\n";
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "<table class=\"table table-hover table-responsive\">\n  <thead>\n    <tr>\n      <th>\n        Item Name\n      </th>\n      <th>\n        Unit of Measure\n      </th>\n      <th>\n        Quantity per unit\n      </th>\n    </tr>\n  </thead>\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.items : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "</table>\n";
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;
  ...
  return "    <tr data-row=\""
    ...
    + "\n      </td>\n      <td>\n        <button title=\"Add to a list\" data-item=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" class=\"btn-sm btn-warning\">&#43;\n        </button>\n      </td>\n    </tr>\n    <tr class=\"hidden\" data-item=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n      <td colspan=\"4\" data-item=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n        <td>\n    </tr>\n";
},"4":function(container,depth0,helpers,partials,data) {
    return "<h4>No items matched your search.</h4>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.items : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.program(4, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
  ...
  return "<form id=\"edit-item-form\" data-id=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.id : stack1), depth0))
    + "\">\n  <h1>Edit Item</h1>\n  <table class=\"table table-hover table-responsive\">\n    <thead>\n      <tr>\n        <th>\n          Item Name\n        </th>\n        <th>\n          Unit of Measure\n        </th>\n        <th>\n          Quantity per Unit\n        </th>\n      </tr>\n    </thead>\n    <tr>\n      <td>\n        <input type=\"text\" name=\"item[name]\" placeholder=\"Item Name\" value=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.name : stack1), depth0))
    + "\" >\n      </td>\n      <td>\n        <input type=\"text\" name=\"item[unit]\" placeholder=\"Unit of Measure\" value=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.unit : stack1), depth0))
    + "\">\n      </td>\n      <td>\n        <input type=\"text\" name=\"item[unit_quantity]\" placeholder=\"Item Name\" value=\""
    + alias2(alias1(((stack1 = (depth0 != null ? depth0.item : depth0)) != null ? stack1.unit_quantity : stack1), depth0))
    + "\">\n      </td>\n    </tr>\n  </table>\n  <input class=\"btn-primary\" type=\"submit\" value=\"Change &raquo;\">\n  <button data-item-index class=\"btn-default\">Cancel\n  </button>\n</form>\n<div id=\"new-item-response\"></div>\n";
},"useData":true});
...
var onSearchItems = function onSearchItems(data) {
  return $.ajax({
...
var onSearchSuccess = function onSearchSuccess(response, status, xhr) {
  handlebars.searchItemResults(response.items);
};
var onSearchFailure = function onSearchFailure(response, status, xhr) {};
...
var onCreateSuccess = function onCreateSuccess(response, status, xhr) {
  if (store.list) {
    var data = { 'list_item': { 'list_id': store.list, 'item_id': response.item.id, 'purchased': false, 'quantity': 1 } };
    liApi.onAddItemToList(data).then(liUI.onCreateSuccess).catch(liUI.onCreateFailure);
  } else {
    $('#new-item-response').empty();
    $('#new-item-form input[type="text"]').val('');
    $('#new-item-form input[type="number"]').val('');
    var html = '</p>Your new item, <strong>' + response.item.name + '</strong>, has been saved.</p>';
